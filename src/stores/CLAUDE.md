...
**`addedBy` reassignment is a re-add, not an edit.** `restoreItem(id, qty, aisle, allocation)` — used both by the undo-adjacent "re-add sheet" and by picking a previously-purchased (done) item from the Add-item sheet's suggestion picker — un-ticks a done item and, like `toggleDone`'s uncheck path, reassigns `addedBy` to the current user: bringing a bought item back onto the list makes the re-adder responsible for it again. `updateItem`, by contrast, is a plain edit (used both from the main list's edit action and from the Add-item sheet's picker when the selected suggestion is *not* done — `ShoppingView.vue`'s `selectMatchingSuggestion` routes a not-done pick straight into edit mode) and deliberately never touches `addedBy`.
...
- `visibleItems` — items filtered by the selection: "All items" shows everything; a specific store shows items allocated to it, plus (unless `storeOnlyFilter` is on) items allocated to all supermarkets (`allSupermarkets`) or unallocated (`supermarketIds` absent/empty). Allocation fields are read defensively so pre-Part-B items behave as unallocated.
- `storeOnlyFilter` (Part C, issue #137) — a boolean toggled by `setStoreOnlyFilter(value)` that, only while a specific supermarket is selected, hides the "all supermarkets"/unallocated items from `visibleItems`, leaving just items allocated to that store. Local view state, **not persisted** — `selectSupermarket` resets it to `false` on every selection change (including re-selecting the same store from the UI) and `teardown()` clears it. `ShoppingView.vue` shows a filter icon button (only when a store is selected) that toggles it.
