...
- **Aisle ordering** — each item carries a single aisle **name**; aisle *ordering* is per-supermarket (issue #137 Part B) and resolved at render from the selected store's `aisles` (the "All items" view uses a deduplicated union). `sortOrder` (drag position within an aisle) is global. Items whose aisle is absent from the selected store fall to the bottom. `aisleOrder` on items is a denormalised fallback for old clients and no longer drives sorting; the store still falls back to `DEFAULT_AISLES` when no supermarkets/aisles exist. See `src/stores/CLAUDE.md`.
- **Bottom sheets with text inputs** — on Android, dismissing the virtual keyboard leaves `v-bottom-sheet` content (bottom-anchored) below the visible screen edge. Apply `useKeyboardAwareSheet(sheetRef, '--css-var-name')` from `src/composables/useKeyboardAwareSheet.js` to every `v-bottom-sheet` that contains a text input. Add `content-class="your-overlay-class"` to the `v-bottom-sheet` and an **unscoped** CSS rule `.your-overlay-class { margin-bottom: var(--css-var-name, 0px); transition: margin-bottom 0.15s ease; }`. Also add `max-height: 92dvh; overflow-y: auto` to the card inside the sheet. `v-dialog` is **not** affected — it uses centred positioning. See `ShoppingView.vue` for three worked examples and issues #49 / #109.
- **Keeping a focused field visible above the keyboard on a tall sheet** — the root cause is that Android's on-screen keyboard shrinks only the **visual** viewport; the **layout** viewport (`window.innerHeight`, and therefore `vh` *and* `dvh`) stays at full height. Note that `dvh` does **not** account for the keyboard — it accounts for retracting browser UI like the address bar. Anything that measures the layout viewport therefore sees no keyboard at all, and anything that scrolls is scrolling a viewport that never shrank. Fix the split rather than compensating for it: `index.html` sets `interactive-widget=resizes-content` on the viewport meta, which makes the keyboard shrink the layout viewport too. Once that holds, `dvh` means what you'd expect and the layout is purely declarative — `.add-item-overlay` (the **unscoped** `content-class` rule, which targets Vuetify's `.v-overlay__content`) is `height: min(92dvh, calc(100dvh - var(--add-item-sheet-bottom, 0px)))`, `.add-item-card` is `height: 100%; display: flex; flex-direction: column`, `.add-item-card > *` is `flex: 0 0 auto` so fields and buttons keep their natural height, and `.add-item-card > .chip-picker-section` is `flex: 1 1 auto; min-height: 0; overflow-y: auto` so the chip picker is the only thing that gives way as the sheet shrinks. The card itself keeps `overflow-y: auto` as an escape hatch: below roughly 320px of layout viewport (landscape with the keyboard up, split-screen) the picker is already at 0 and the fixed-height fields and button row still overflow — with `overflow: hidden` they were clipped and the Add button became unreachable (measured at 250px: a real wheel gesture moved `scrollTop` 0px). No JS measures anything. **The dvh height must sit on the overlay wrapper, not the card** — `.v-overlay__content` is a flex container whose own height is content-driven, so a `dvh` height on the card alone is silently ignored and the sheet stays content-sized (measured: 540px instead of 776px on an 844px viewport). **The `min()`/`calc()` around the `dvh` is not decoration** — on engines that ignore `interactive-widget` (WebKit, and Chromium before 108, which some Fire tablets still ship as Silk) `dvh` stays at full height while `useKeyboardAwareSheet` keeps reporting the real keyboard height, and a bare `height: 92dvh` would *stack* with that `margin-bottom` and drive the sheet off the top of the screen (measured: overlay top at −268px on an 844px viewport with a 336px keyboard — #162's original symptom, and worse than the content-sized card it replaced). Subtracting the same variable inside the height makes them compose instead. Where the meta tag is honoured the variable is `0px` and it resolves to plain `92dvh`. **Do not remove the `interactive-widget` meta tag** — the sheet's sizing depends on it, and without it the sheet extends underneath the keyboard again with no other visible symptom. `useKeyboardAwareSheet` is retained for the #49/#109 bottom-anchoring fix and self-neutralises under `resizes-content` (both viewport heights converge, so it emits `0px`).
  - **You cannot validate this locally — verify on a physical Android device.** Faking a keyboard by resizing the window (jsdom, Playwright, Chrome DevTools device mode, `Emulation.setDeviceMetricsOverride`) shrinks *both* viewports together, which removes the exact discrepancy being tested and makes broken code pass. Measured directly: a page with the meta tag and one without produce byte-identical `innerHeight`/`visualViewport.height`/`100dvh` readings under a simulated resize. There is also no way to drive `env(keyboard-inset-height)`. Treat any green local "keyboard test" as meaningless. The tests in `ShoppingView.test.js` under *keyboard-aware sheet sizing* deliberately guard only the wiring (meta tag present, `dvh` sizing, flex chain, no JS measurement), each of which is silently removable, and say so in a comment.
  - **Superseded approaches** (kept because the failure modes are non-obvious and easy to retry). Four reactive approaches were tried for issue #162 (Item name scrolled out of view on the Add item sheet when a family has many aisles/supermarkets) and none held up on a real Android device: `Element.scrollIntoView()` on the focused field (judges visibility against `window.innerHeight`, which doesn't shrink for the keyboard — only `window.visualViewport` does); a manual `visualViewport`-based `scrollTop` correction (same root problem, still didn't work on-device); a fixed-header/separately-scrolling-middle/fixed-footer split (with no scrollable ancestor near the field, the browser escalated to scrolling the *outer page*, making it worse); and sticky header/footer within one scroll region (still didn't fix it on-device). All four shared the same root cause described above. A fifth approach then avoided the keyboard entirely by bounding the sheet's height so it could never reach the keyboard: the Aisle and Available in pickers were capped in one shared scrolling region at a hand-measured 236px budget (`CHIP_PICKER_MAX_HEIGHT`), reduced at runtime by the Re-add row's live `offsetHeight` and floored at 58px. That worked, but it kept the sheet permanently small on every device regardless of available space, and the hand-measured constants had to be re-measured whenever Vuetify's chip size or the spacing scale changed. It was removed once `interactive-widget` made the viewport itself keyboard-aware.
