...
- **Keeping a focused field visible above the keyboard on a tall sheet** — do NOT try to fix this with JS (`scrollIntoView`, or a manual `visualViewport`-based scroll correction) — both were attempted and proved unreliable on real Android devices (issue #162; `scrollIntoView` in particular judges visibility against `window.innerHeight`, which does not shrink for the keyboard, only `window.visualViewport` does, so it can wrongly conclude a hidden field is "already visible"). Instead, structure the card as a flex column with a **fixed header** for text inputs (title + any `v-text-field`s), a **scrollable middle** (`flex: 1 1 auto; min-height: 0; overflow-y: auto`) for tap-only content (chip pickers, suggestion lists), and a **fixed footer** for action buttons — the card itself gets `overflow: hidden` instead of `overflow-y: auto`. Since the fields you type into never sit inside the scrolling region, the keyboard has nothing to scroll them behind. See `ShoppingView.vue`'s Add item sheet (`.add-item-header` / `.add-item-scroll` / `.add-item-footer`).
