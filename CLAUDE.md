...
- **Keeping a focused field visible above the keyboard on a tall sheet** — don't try to react to the keyboard at all; cap what makes the sheet tall in the first place. Four reactive approaches were tried for issue #162 (Item name scrolled out of view on the Add item sheet when a family has many aisles/supermarkets) and none held up on a real Android device: `Element.scrollIntoView()` on the focused field (judges visibility against `window.innerHeight`, which doesn't shrink for the keyboard — only `window.visualViewport` does); a manual `visualViewport`-based `scrollTop` correction (same root problem, still didn't work on-device); a fixed-header/separately-scrolling-middle/fixed-footer split (with no scrollable ancestor near the field, the browser escalated to scrolling the *outer page*, making it worse); and sticky header/footer within one scroll region (still didn't fix it on-device). What worked: bound the Add item sheet's height regardless of how many aisles/supermarkets a family has. First cut capped the Aisle and Available in chip rows independently at two lines each with their own scrollbar — functional, but two small stacked scroll boxes was poor UX. Current version (`.chip-picker-section` in `ShoppingView.vue`) wraps both pickers in **one** shared scrolling region instead: each chip row wraps freely (no per-row cap), and only the combined region scrolls, as a single scrollbar, once its content exceeds a `max-height` budget of 236px — the footprint the two independently-capped boxes used to occupy together (measured: 98px Aisle box + 12px gap + 126px Available in box). Nothing ever needs to be scrolled to keep the fields above it visible in the first place. That 236px budget assumes no "Re-add" suggestion row above it (shown when the typed item name matches a done item); since that row's height varies with how many suggestions matched and how long their names are — not a fixed quantity like the aisle/supermarket rows — its live height is measured from the DOM (`reAddSectionEl.offsetHeight`, `<script setup>`) and subtracted from the 236px budget reactively (`chipPickerMaxHeight`), rather than hard-coded, so the sheet's total height stays constant whether or not the Re-add row is showing. That subtraction is floored at `CHIP_PICKER_MIN_HEIGHT` (58px, measured — the "Aisle" label plus one chip row) rather than 0, so a large Re-add row (up to 5 suggestions, possibly wrapping to 2-3 lines) can shrink the picker a lot but never below room to see and tap at least one aisle/supermarket chip.
