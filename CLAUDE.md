...
| `src/stores/__tests__/pocketMoney.test.js` | Unit | `pendingPaymentDates`, `displayBalance`, `setup` (parent vs child), `teardown`, `flushPendingPayments`, `saveConfig`, `recordWithdrawal`, `loadTransactions`. Date math is verified with **fake timers pinned to UTC** and **parameterised exact assertions** — month/year boundaries, leap day, multi-week gaps, all seven payment days, DST transitions, and ledger accuracy (exact balance + one dated transaction per payment) |
