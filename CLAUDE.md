...
- **Bottom sheets with text inputs** — on Android, dismissing the virtual keyboard leaves `v-bottom-sheet` content (bottom-anchored) below the visible screen edge. Apply `useKeyboardAwareSheet(sheetRef, '--css-var-name')` from `src/composables/useKeyboardAwareSheet.js` to every `v-bottom-sheet` that contains a text input. Add `content-class="your-overlay-class"` to the `v-bottom-sheet` and an **unscoped** CSS rule `.your-overlay-class { margin-bottom: var(--css-var-name, 0px); transition: margin-bottom 0.15s ease; }`. Also add `max-height: 90dvh; overflow-y: auto` to the card inside the sheet. `v-dialog` is **not** affected — it uses centred positioning. The composable also re-scrolls whichever field currently has focus into view (on focus and on every `visualViewport` resize), so a field near the top of a long, internally-scrolling card (e.g. many aisle/supermarket chips) doesn't get scrolled out above the keyboard — no per-field wiring needed. See `ShoppingView.vue` for three worked examples and issues #49 / #109 / #162.
