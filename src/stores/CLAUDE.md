...
**`shopping` store specifics:** `setup(familyId)` subscribes to the `families/{familyId}/shoppingLists` subcollection to get list metadata — the family scope is in the path (there is no `familyId` field on list documents, and no `where()` filter), and it does **not** auto-create any document. All list/item reads and writes go through the path helpers (`listsCol`/`listDoc`/`itemsCol`/`itemDoc`) which prepend `families/{currentFamilyId}`. When the snapshot fires with results, the store auto-activates the most recently created list. `activateList(listId)` starts a second listener on that list's items subcollection. `createList(name)` is a parent-only action that creates a new list document (with a default `aisles` array) and activates it. `reorderItems(updates)` is a parent-only action (enforced in the UI) that batch-writes `sortOrder` (and optionally `aisle`/`aisleOrder` for cross-section moves) to persist drag-and-drop order. `saveAisles(aisles)` is a parent-only action that writes the current aisle list to the list document. `deleteAisle(name)` is a parent-only action that batch-moves items in the deleted aisle to `aisle: 'Unknown'` and removes the aisle from the list document. `toggleDone(id)` flips an item's `done` flag; on the check path it also clears `priority` to `false` (a done item is no longer priority); on the uncheck path it reassigns `addedBy` to the current user (unchecking means "I want this again"). Because of these side effects, `toggleDone` is **not** a clean inverse of itself — so the undo affordance in `ShoppingList.vue` does not re-toggle. Instead the component captures the pre-toggle `{ done, addedBy, priority }` and, on Undo, calls `restoreToggleState(id, { done, addedBy, priority })`, which writes those exact values back so an accidental tick is reverted without re-attributing the item or losing its star. `activeAisles` is a computed that returns the active list's `aisles` array, falling back to `DEFAULT_AISLES` if the field is absent (old documents). `teardown()` cleans up both the lists and items listeners.
