...
- **Keeping a focused field visible above the keyboard on a tall sheet** — do NOT try to fix this with JS (`scrollIntoView`, or a manual `visualViewport`-based scroll correction) — both were attempted and proved unreliable on real Android devices (issue #162; `scrollIntoView` in particular judges visibility against `window.innerHeight`, which does not shrink for the keyboard, only `window.visualViewport` does, so it can wrongly conclude a hidden field is "already visible"). A structural attempt that split the card into a non-scrolling header + a separately-scrolling middle also failed: with no scrollable ancestor near the focused field, the browser's native focus-into-view behaviour escalated to scrolling the *outer page*, dragging the whole sheet off-screen — worse than the original bug. **What works:** keep the card as ONE `overflow-y: auto` region (a real, nearby scrollable ancestor for the browser to target), and make the header (title + text inputs) and footer (action buttons) `position: sticky` (`top: 0` / `bottom: 0` respectively, with `background: rgb(var(--v-theme-surface))` so scrolled content doesn't show through) *within* that single scrolling card. Sticky elements can't be scrolled past regardless of what triggers the scroll — user gesture or the browser's own auto-scroll. See `ShoppingView.vue`'s Add item sheet (`.add-item-card` / `.add-item-header` / `.add-item-body` / `.add-item-footer`).
