...
describe('Firestore security rules', () => {
  ...
  describe('meals collection', () => {
    ...
    it('allows a child to create a meal', async () => {
      const ctx = testEnv.authenticatedContext('child-uid')
      await assertSucceeds(
        addDoc(collection(ctx.firestore(), 'meals'), {
          familyId: 'fam-1', name: 'Tacos', votes: [], ingredients: [],
        })
      )
    })

    it('allows a parent to create a meal', async () => {
      const ctx = testEnv.authenticatedContext('parent-uid')
      await assertSucceeds(
        addDoc(collection(ctx.firestore(), 'meals'), {
          familyId: 'fam-1', name: 'Curry', votes: [], ingredients: [],
        })
      )
    })

    it('denies a non-member creating a meal', async () => {
      const ctx = testEnv.authenticatedContext('outsider-uid')
      await assertFails(
        addDoc(collection(ctx.firestore(), 'meals'), {
          familyId: 'fam-1', name: 'Stolen Meal', votes: [], ingredients: [],
        })
      )
    })

    it('allows a family member to update the votes field (vote on a meal)', async () => {
      const ctx = testEnv.authenticatedContext('child-uid')
      await assertSucceeds(
        updateDoc(doc(ctx.firestore(), 'meals', 'meal-1'), {
          votes: ['child-uid'],
        })
      )
    })

    it('denies a non-member updating a meal', async () => {
      const ctx = testEnv.authenticatedContext('outsider-uid')
      await assertFails(
        updateDoc(doc(ctx.firestore(), 'meals', 'meal-1'), {
          votes: ['outsider-uid'],
    ...
    it('cross-family: member of another family cannot read fam-1 meals', async () => {
      await seedFamily('fam-2', [
        { uid: 'other-parent', name: 'Other', role: 'parent' },
      ])
      const ctx = testEnv.authenticatedContext('other-parent')
      await assertFails(getDoc(doc(ctx.firestore(), 'meals', 'meal-1')))
    })

    it('cross-family: member of another family cannot create meals for fam-1', async () => {
      await seedFamily('fam-2', [
        { uid: 'other-parent', name: 'Other', role: 'parent' },
      ])
      const ctx = testEnv.authenticatedContext('other-parent')
      await assertFails(
        addDoc(collection(ctx.firestore(), 'meals'), {
          familyId: 'fam-1', name: 'Injected', votes: [], ingredients: [],
        })
      )
    })
  })
